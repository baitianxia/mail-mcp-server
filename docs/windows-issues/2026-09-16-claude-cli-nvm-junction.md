...
待用户提供 `where node` 后确认 Node 的最终版本目录；现有代码已经确认存在一个确定的错误：
...
- Windows 标准用户门禁使用目录联接 fixture，覆盖 NVM 类型的路径。

## 验证结果

- macOS 本地单测：86/86 通过。
- Windows gate：待本修复提交后运行。
- 用户实际路径：`where.exe claude` 已确认；`where node` 尚未提供。

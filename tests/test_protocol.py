...
class ProtocolTests(unittest.TestCase):
    def test_mcp_layout_is_portable_without_a_skill_install(self) -> None:
        ...
        self.assertIn("CLAUDE DISCOVERY", installer)
        self.assertIn("rejected after validation", installer)
        ...
        self.assertIn("WinGet\\Links", discovery)
        self.assertIn("where.exe", discovery)
        self.assertIn("Resolve-CoremailExternalFilePath", discovery)
        self.assertIn("GetFinalPathNameByHandle", discovery)
        ...
        self.assertIn("where.exe", lifecycle_gate)
        self.assertIn("WinGet Links", lifecycle_gate)
        self.assertIn("Resolve-ClaudeCodeInvocation", lifecycle_gate)
        link_gate = (ROOT / "tests" / "windows-cli-discovery.ps1").read_text(encoding="utf-8")
        self.assertIn("ItemType Junction", link_gate)
        self.assertIn("ItemType SymbolicLink", link_gate)
        self.assertIn("2\\.1\\.84", link_gate)
